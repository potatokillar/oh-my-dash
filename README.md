...
- **`dsh-mobile-react/`** — React + Capacitor Android 客户端（当前主产物，v2.0.0 起）：多设备管理（设备 → 项目/会话 → 对话）、项目（workspace）列表、服务器端目录浏览、远程审批（允许一次/拒绝）、实时事件流同步（手机发的消息 PC 浏览器可见，反之亦然）。
- **`dsh_mobile/`** — 旧 Flutter Android 客户端：已废弃，仅作协议实现参考保留。
